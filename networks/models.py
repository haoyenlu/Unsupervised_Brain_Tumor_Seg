    ...
    def forward(self, U, I):
        ...
        for _ in range(self.n_iter):
            ...
            Q = self.compatibilitytransf(Q) \
                - Q * self.compatibilitytransf.weight
            #adding unary
            Q = U - Q
        return Q
    
class CRFRNN_ViT(VisionTransformer):
  def __init__(self,vit_config,num_classes=1,image_size=512):
    super(CRFRNN_ViT,self).__init__(vit_config,img_size=image_size,num_classes=num_classes)
    self.crfrnn = CRFRNN(n_iter=10,in_channels=3,n_classes=1)

  def forward(self,X):
    out = super(CRFRNN_ViT, self).forward(X)
    return self.crfrnn(out, X)
  
  def _get_transformer_output(self,X):
    return super(CRFRNN_ViT,self)._get_transformer_output(X)